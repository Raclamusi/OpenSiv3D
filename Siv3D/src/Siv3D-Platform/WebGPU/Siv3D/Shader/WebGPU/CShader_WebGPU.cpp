...
namespace s3d
{
	...
	void CShader_WebGPU::init()
	{
		...
		// エンジン VS をロード
		{
			m_engineVSs << WGSL{ Resource(U"engine/shader/wgsl/quad_warp.vert.wgsl"), {{ U"VSConstants2D", 0 }, { U"VSQuadWarp", 1 }} };

			if (not m_engineVSs.all([](const auto& vs) { return !!vs; })) // もしロードに失敗したシェーダがあれば
			{
				throw EngineError{ U"CShader_WebGPU::m_engineVSs initialization failed" };
			}
		}
		...
		// エンジン PS をロード
		{
			...
			m_enginePSs << WGSL{ Resource(U"engine/shader/wgsl/quad_warp.frag.wgsl"), {{ U"PSConstants2D", 0 }, { U"PSQuadWarp", 1 }} };
	...
	const VertexShader& CShader_WebGPU::getEngineVS(const EngineVS vs) const
	{
	...
	void CShader_WebGPU::setQuadWarpCB(const VS2DQuadWarp& vsCB, const PS2DQuadWarp& psCB)
	{
